...
That estimate was later updated to half a million in 2015, causing 15,000 deaths. \cite{cdc2018} \cite{cdc2015} 
...
For fiscal years 2013 to 2018, the following formula is used to calculate payment adjustment:

\begin{equation} \label{eq1}
\begin{split}
  \text{Payment Readjustment Factor} &= 1 - min{0.03, \sum_{dx} \frac{\text{Payment(dx) \cdot max\big((\text{ERR}(dx) - 1.0), 0\big)}}{\text{All payments}}} \\
\end{split}
\end{equation}
...
ERR is a hospital's performance measure $dx$, and payment refers to base operating DRG payments. \cite{HRRPPaymentAdjustment}
